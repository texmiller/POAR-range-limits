...
\textit{Poa arachnifera} is a perennial, cool-season (C3) grass endemic to the southern Great Plains. 
This species occurs almost exclusively in central Texas, Oklahoma, and southern Kansas (Fig. \ref{fig:map}) though there are occasional records of adventive populations in other states\footnote{http://bonap.net/Napa/TaxonMaps/Genus/County/Poa}.
...
We fit a binomial generalized linear model (glm), where females were ``successes'' and total inflorescences was the number of ``trials'', to test whether the OSR varied systematically with respect to longitude. 
