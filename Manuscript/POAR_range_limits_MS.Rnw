...
Here we ask, for the first time to our knowledge, whether female demographic responses to environmental variation, alone, are sufficient to understand the ecological origins of range limits, or whether the additional role of males determines range boundary formation. 
...
Source material for this experiment came from \Sexpr{poar_ms_quantities$n_sources} sites, which were a subset of the sites that were visited for the natural population survey (Table \ref{tab:survey}). 
...
As a general rule, in long-lived species with polygamous mating systems, range limits should reflect the niche limits of females. Polygyny implies that reproduction will be affected by sex ratios only at very high proportion of females. This is exemplified by our results, where seed production started to decrease only in populations comprised of 70\% females. Moreover, in long-lived species population growth rates will depend only weakly to reproduction \citep{franco2004comparative}. These two characteristics will substantially decrease the likelihood of males setting range limits.
