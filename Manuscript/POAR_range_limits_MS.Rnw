...
library(tidyverse)
library(xtable)
poar_ms_quantities <- read_rds("C:/CODE/POAR-range-limits/Manuscript/poar_ms_quantities.rds")
@
...
\section*{Abstract}
\linenumbers
Understading the mechanisms that generate biogeographic patterns of distribution and abundance is a long-standing goal of ecology. 
...
In dioecious species, for example, sexual niche differentiation may cause divergence between the sexes in their limits of environmental suitability. 
We studied the mechanisms of range boundary formation in the perennial dioecious grass \textit{Poa arachnifera}, testing the alternative hypotheses that range limits reflect the niche limits of females only, as assumed by classic ecological theory, versus the combined contributions of females and males, including their inter-dependence via male-limitation of female fertility. 
Common garden experiments across the longitudinal aridity gradient of the southern Great Plains, US revealed female-biased flowering and panicle production approaching eastern range limits, consistent with surveys of operational sex ratio variation in natural populations. 
A process-based demographic model predicted longitudinal limits of population viability ($\lambda \geq 1$) that matched the observed eastern and western range limits, and further showed that declines in $\lambda$ approaching range limits were driven almost entirely by declines in female vital rates. 
Thus, despite the potential for mate limitation particularly at eastern margins, the geographic distribution was effectively female-dominant, reflecting the environmental niche of females with little contribution from males. 
The dominant role of females was attributable to female fertility being quite robust to sex ratio variation (which declined only at extreme under-representation of males) and to relatively low sensitivity of $\lambda$ to reproductive transitions in the life cycle.
This suggests that female-dominant limitation of geographic distribution may be common to long-lived species with polygamous mating systems. 
...
The niche concept is central to theory for range limits \citep{Hutchinson1958} and available evidence suggests that geographic distributions may commonly be interpreted as ecological niches ``writ large'' \citep{lee2016synthesis,hargreaves2013species}. 
...
The mechanistic understanding offered by process-based models of range limits provides a potentially powerful vehicle for predicting range shifts in response to current and future environmental change. These models, beyond improving predictions, will allow predicting abundances and how fast they will respond to environmental change \citep{evans2016towards,ehrlen2015predicting}. 
...
Intra-specific niche differences may be based on demographic state variables such as life stage, size class or other, unmeasured aspects of individual identity. 
...
There are now numerous examples of sex differences in trophic position \citep{pekar2011intersexual,law2018carnivory}, habitat use \citep{bowyer2004sexual,phillips2004seasonal}, and responses to climate \citep{petry2016sex,rozas2009sex,gianuca2019sex}, differences that may or may not be accompanied by sexual dimorphism. 
It has been hypothesized that sex-specific niches may evolve by natural selection when it reduces competitive or other antagonistic interactions between the sexes \citep{bierzychudek1988spatial, bolnick2003sexual} or as a byproduct of naturally or sexually selected size dimorphism \citep{shine1989ecological,temeles2010evolution}.

Sexual niche differentiation can translate to sex-specific advantages in different environments, causing skew in the operational sex ratio (OSR: relative abundance of females and males available for mating) even if the primary (birth) sex ratio is unbiased \citep{veran2009demographic,shelton2010ecological,eberhart2017sex}.
Indeed, environmental clines in OSR have been widely documented in plants and animals at fine spatial scales \citep{eppley2001gender,bertiller2002spatial,groen2010sex} as well as broader climatic clines across alititutdes or latitudes \citep{petry2016sex,ketterson1976geographic,caruso2007sex}. 
At range margins, where environments are extreme relative to the range core,
demographic differences between the sexes, and hence skew in the OSR, may be greatest. 
In dioecious plants, for example, populations at the upper altitudes and latitudes and in the more xeric margins of species’ ranges tend to be male-biased \citep{field2013ecological}.

Returning to the question of whose niche determines range limits given potential for sexual niche differentiation, classic ecological theory assumes the answer. 
...
This assumption is, of course, wrong but it may often be a convenient approximation when the sex ratio is balanced or does not vary. 
...
However, females may be male-limited in environments in which they are favored, which could reduce population viability in marginal environments. 
...
% add sentence about how sex specific niches does not guarantee an important role for males, since few males are required in polygamous mating systems and, for slow life cycles, reproductive effects often have weak sensitivities.
...
\begin{figure}[H]
  ...
  In this case, expectations for range limits may differ from the female-dominant prediction, since mate limitation in environments that favor females over males may reduce population viability. 
...
Here we ask, for the first time, whether female demographic responses to environmental variation, alone, are sufficient to understand the ecological origins of range limits, or whether the additional role of males determines range boundary formation. 
As an experimental model, we worked with a dieocious plant species (the grass \textit{Poa arachnifera}) narrowly distributed across the sharp longitudinal aridity gradient in the southern Great Plains, US (Fig. \ref{fig:map}). 
...
We hypothesized that sexual niche differentiation with respect to longitudinal variation in aridity may lead to skewed sex ratios approaching range limits, and that mate limitation could cause range boundaries to deviate from female-dominant expectations. 

This study was conducted in four parts. 
First, we conducted surveys to ask whether natural populations exhibit clines in operational sex ratio across longitudinal environmental variation. 
...
Third, we conducted a local sex ratio manipulation experiment to quantify how viable seed production by females responds to variation in sex ratio. 
...
\textit{Poa arachnifera} is a perennial, cool-season grass endemic to the southern Great Plains. 
This species occurs almost exclusively in central Texas, Oklahoma, and Kansas (Fig. \ref{fig:map}) though there are occasional records of adventive populations in other states\footnote{http://bonap.net/Napa/TaxonMaps/Genus/County/Poa}.
...
We surveyed \textit{P. arachnifera} across its range to establish whether natural populations exhibited geographic clines in operational sex ratio corresponding to the longitudinal aridity gradient. 
...
At each location, we searched for \textit{P. arachnifera} along roads, trails, or creek drainages and recorded the number of female and male patches that we encountered and the number of inflorescences on each patch. 
...
We fit a binomial generalized linear model (glm), where females were ``successes'' and total inflorescences was the number of ``trials'', to test whether the operational sex ratio varied systematically with respect to longitude. 
...
This statistical model and all those that follow were fit in a Bayesian statistical framework using Stan \citep{carpenter2017stan} and rstan \citep{stan2018rstan} with vague priors on all parameters.
In all cases, model fit was assessed with posterior predictive checks \citep{conn2018guide}. 
...
We established a common garden experiment at \Sexpr{poar_ms_quantities$n_sites} sites throughout and beyond the geographic distribution of \textit{P. arachnifera} (Fig. \ref{fig:map}).
Experimental sites spanned latitudinal and longitudinal variation, though we focus here on longitude. 
During the three years of this experiment, total precipitation at each site closely tracked longitude (Fig. \ref{fig:site_precip}), as expected based on longer-term climate trends (Fig. \ref{fig:map}).
Source material for this experiment came from \Sexpr{poar_ms_quantities$n_sources} sites, which were of subset of the sites that were visited for the natural population survey (Table \ref{tab:survey}). 
At these sites, we collected tillers from flowering individuals of each sex (mean: \Sexpr{round(mean(poar_ms_quantities$source_plants$n),1)} individuals per site, range: \Sexpr{min(poar_ms_quantities$source_plants$n)}--\Sexpr{max(poar_ms_quantities$source_plants$n)}). 
These were brought back to the Rice University greenhouse, where they were clonally propagated in ProMix potting soil and supplemental Osmocote fertilizer at 78--80$^{\circ}$F under natural humidity and light. 

Common gardens were set up in Fall (October--December) 2014. 
At each site, we established \Sexpr{poar_ms_quantities$n_block} experimental blocks, which typically corresponded to a tree or woodland edge, providing partial shade that mimics this species' natural micro-environment. 
...
We pooled all three years of observations for analysis so we do not explicitly model temporal variation but our results are implicitly averaged over years. 
...
For growth, we modeled tiller number in year $t+1$ with a zero-truncated Poisson-Inverse Gaussian (PIG) distribution. 
...
At one site near the center of the range (Lake Lewisville Environmental Learning Area in North-central Texas), we established a separate experiment to quantify how sex ratio variation affects female reproductive success.
Details of this experiment, which was conducted in 2014--2015, are described in Compagnoni et al. \citeyear{compagnoni2017can}. 
Briefly, we established 124 experimental populations in $0.4 m \times 0.4 m$ plots that varied in population density (1--48 plants/plot) and sex ratio (0--100\%female), with 2--4 replicates each of 34 density-sex ratio combinations. 
...
In the lab, we counted the total number of seeds on each panicle and assessed seed viability in the greenhouse with germination trials of 25 seeds per panicle. 
We also conducted tetrazolium-based seed viability assays (17--57 seeds per panicle, mode: 30). 
...
\noindent where $OSR$ is the operational sex ratio (fraction of panicles that were female) in our experimental populations. The OSR therefore quantifies the availability of males to each female individual in a population.
This function has the properties, supported by our previous work, that seed viability is maximized at $v_{0}$ as $OSR$ approaches zero and goes to zero as $OSR$ approaches $1$.
...
The statistical models for the common garden and sex ratio experiments provided the backbone of the full demograhpic model, a matrix projection model (MPM) structured by size (tiller number) and sex. 
...
We used the seedling survival probability ($\sigma$) from our demographic studies of the perennial congener \textit{Poa autumnalis} in east Texas (T.E.X. Miller and J.A. Rudgers, \textit{unpublished data}) as a stand-in for \textit{P. arachnifera}, and we assume this probability was constant across sexes and longitudes ($\sigma = \Sexpr{poar_ms_quantities$poau_sdlg_surv}$). 
...
\begin{figure}%[h!]
  ...
  Lower panels below each data panel show the posterior distribution of the difference between females and males as a function of longitude (positive and negative values indicate female and male advantage, respectively); dashed horizontal line shows zero difference.}
  \label{fig:vital_rates}
  \end{center}
\end{figure}

\subsection*{Geographic variation in sex-specific demography}
In year one, there was near-total mortality of transplants in the common garden experiment at three sites due to various catastrophes (a flood, a drought, a pack of voles); otherwise, there was good establishment in the common gardens. 
...
In Fig. \ref{fig:vital_rates}, we show binned means of raw data and fitted vital rate models for four vital rates (rows) and three size classes (columns), discretized for visualization only. 
...
Sex differences in flowering and panicle production generated a longitudinal trend in the operational sex ratio of our common garden populations consistent with (but quantitatively weaker than) the trend in natural populations: the fraction of total panicles that were female in our common gardens increased from west to east (Fig. \ref{fig:survey}B) even as the fraction of surviving plants that were female did not show a longitudinal trend (Fig. \ref{fig:survey}C).
Thus, in recapitulating the natural OSR pattern, the common garden experiment suggested that the longitudinal trend in the mating pool of natural populations was due to the reproductive niche of females extending farther east than that of males, and not to sex differences in mortality.
...
Specifically, the western-most and eastern-most county records of \textit{P. arachnifera} fell within the uncertainty distribution of the model's predictions (represented by the shading in Fig. \ref{fig:lambda_long}A), bolstering our confidence that the model effectively captured geographic patterns in the population dynamics of this species. 
Also, the asymptotic population structure predicted by the model showed skew in the operational (panicle) sex ratio with increasing female bias toward the eastern range margins, consistent with observations in the field (Fig. \ref{fig:SR_viab_sens}A). 
...
LTRE decomposition methods revealed that declines in $\lambda$ approaching range limits were driven almost exclusively by females (Fig. \ref{fig:lambda_long}B) with near-zero contributions from males (Fig. \ref{fig:lambda_long}C). 
Thus, range limitation was an effectively female-dominant process, despite systematic geographic variation in sex ratio. 
Correspondingly, predictions of the two-sex model were nearly indistinguishable from a corresponding female-dominant model with all else equal except that female seed fertilization did not depend on males, with only very modest differences in predictions of the two models emerging in the eastern part of the range  (Fig. \ref{fig:2sex_Fdom}). 
...
Skew in the OSR predicted by the demographic model was less extreme than was observed in natural and experimental populations. 
...
While these changes increased the contributions of males to range limitation, female contributions were still more than twice as important as males, and there was very little difference between predictions of the two-sex and female-dominant models under this elevated growth scenario (Fig. \ref{fig:elevated_growth_model}). 
This leads us to conclude that, while our common garden results may quantitatively underestimate OSR bias and its demographic consequences, our qualitative conclusion that range boundary formation is effectively female-dominant in this system is robust to any biases imposed by the size distributions of common garden populations. 
...
Understanding the sources of decline in population viability at range edges is the foundation for predicting how species' ranges will respond to global change drivers. 
Sexual niche differentation has the potential to generate skew in the mating pool across environmental gradients and may therefore contribute to reproductive failure at range edges. 
In Texas bluegrass, we found that the female reproductive niche extended farther east than that of males, generating female-biased operational sex ratios toward the eastern, mesic range margins, a pattern detected in natural populations and recapitulated in our common garden experiment. 
Furthermore, seed viability declined with increasing skew in the OSR, indicating that mate (pollen) limitation can limit female reproductive output in female-biased mating pools. 
It would appear that all the pieces are in place for an important role of two-sex dynamics in generating distributional limits, particularly at the eastern edges of Texas bluegrass. 
Yet, the insights derived from a field-parameterized population model indicate the opposite: range limitation in this species was an effectively female-dominant process, with negligible contributions from males. 
...
As a general rule, in long-lived species with polygamous mating systems, range limits should generally reflect the niche limits of females. Polygyny implies that reproduction will be affected by sex ratios only at very high proportion of females. This is exemplified by our results, where seed production started to decrease only in populations comprised of 70\% females. Moreover, in long-lived species population growth rates will depend only weakly to reproduction \citep{franco2004comparative}. These two characteristics will substantially decrease the likelihood of males setting range limits.

However, the premises for males to contribute in range limit formation, albeit rare, are likely to occur in nature. First, female-skewed sex ratios are common both plants \citep{barrett2010ecological} and animals \citep{caswell2001}. Second, there are several examples of plants responding in contrasting ways to environmental gradients according to sex \citep{bierzychudek1988spatial}. These patterns can be driven by several mechanisms, such as physiology \citep{dawson1999sexual}, differences in competitive ability \citep{eppley2006females}, or demographic rates \citep{delph1999sexual}. Third, mate limitation is common in both animals \citep{gascoigne2009dangerously} and plants \citep{Ashman et al. 2004}. Accordingly, in plants, pollen limitation is considered the reason why dioecious plant species tend to have a substantially smaller range than selfing species \citep{grossenbacher2015geographic}.

Our results reinforce the commonality of demographic compensation in nature, and provides evidence in favor of the abundant center hypothesis of range formation. Demographic compensation occurs when the vital rates of a species have opposing trends across an environmental gradient \citep{villellas2015demographic}. The abundant center hypothesis posits that the fitness and abundance of species will gradually decrease towards range edges \citep{brown1995spatial}. Our results show that in P. arachnifera, survival tended to compensate the negative response of the remaining vital rates at latitudinal extremes. However, this compensation was not sufficient to prevent a decrease in population growth rates at the extremes of the longitudinal range of the species. A recent study found a similar pattern, where compensation between vital rates could not prevent a decrease of population growth rate towards the southern range edge of Erythranthe cardinalis \citep{sheth2018demographic}. Our results and those of Sheth and Angert (2018) might explain why previous tests of the abundant center hypothesis failed to support this hypothesis. These previous tests relied on vital rates rather than population growth rates, providing contrasting results  \citep{sexton2009evolution,abeli2014effects}. Contrasting results among vital rates are expected, because demographic compensation should be adaptive \citep{villellas2015demographic}. Accordingly, the studies that calculate population growth rates along an environmental gradient tend to support the abundant center hypothesis. We know of four such studies in the existing literature. Three of these studies showed a decrease in population growth rates towards at least one end of the environmental gradients \citep{eckhart2011geography,sheth2018demographic,baer2019declining}. The fourth study showed population growth rate along environmental gradients was unpredictable \citep{oldfather2019microclimate}. However, this last study emphasized the importance of microclimate, which can have dramatic effects on alpine plants \citep{scherrer2010infra}. In general, our results suggest that to understand range formation, investigators should rely on population growth rates, or on the entire life cycle of species.


%--------------------------------------------------------------------
\section*{Acknowledgements}

We thank Dariusz Malinowski, Jason Goldman, Tom Arsuffi, Alan Byboth, John Walker, Kenneth Steigman, Steven Gibson, Wesley Newman, Kerry Griffis, Liz Martin, Melanie Hartman, Brian Northup, Dexter R Mardis, and Dixie Smith for assistance in accessing and setting up the field sites. We thank Marion Donald, Kory Kolis, and Nakian Kim for assistance in the field and greenhouse. 

\section*{Author contributions}

A.C. and T.E.X.M. designed the study. A.C., and T.E.X.M. carried out the study. T.E.X.M. and A.C. conducted the statistical analyses and drafted the manuscript. A.C. and T.E.X.M. finalized the manuscript.
...
print(xtable(poar_ms_quantities$survey_site_table, caption = "Sites of natural population surveys corresponding to Figure", label = "tab:survey",include.rownames=F))
@
...
%--------------------------------------------------------------------
\newpage
\section*{Appendix B: Parameter estimates of two-sex MPM}
...
% put the big parameter estimate table here %
...
%--------------------------------------------------------------------
\newpage
\section*{Appendix D: Size distribution comparisons and simulation experiments}
...
In this section, we compare size distributions of natural and experimental populations, and explore how the size distribution predicted by the two-sex IPM affects our conclusions about the role of males in range boundary formation. 
...
\begin{figure}[H]
  ...
  \caption{Relationship between area ($m^3$) and tiller count from plants in the common garden experiment. 
...
In \textbf{C}, the two size distributions come from the base model parameterized following methods described in the main manuscript (gray) and a numerical experiment where growth parameters were numerically increased to generate a size distribution more consistent with natural populations (white). 
  }
  \label{fig:size_dist}
  \end{center}
\end{figure}
