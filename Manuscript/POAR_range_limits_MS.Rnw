...
\begin{center}
  ...
  \textbf{A}, Operational sex ratio (fraction of panicles that were female) in \Sexpr{poar_ms_quantities$n_survey_pops$n} natural populations; \textbf{B}, Operational sex ratio and \textbf{C}, sex ratio (fraction of plants that were female) in \Sexpr{poar_ms_quantities$n_sites} common gardens. 
...
Also, the asymptotic population structure predicted by the model showed skew in the operational (panicle) sex ratio with increasing female bias toward the eastern range margins, consistent with observations in the field (Fig. \ref{fig:SR_viab}A). 
Female bias in the OSR was predicted to cause declines in seed viability toward eastern range margins (Fig. \ref{fig:SR_viab}B).
...
Furthermore, population viability at the eastern range margin was weakly sensitive to seed viability relative to other vital rates (). 
...
However, stable size distribution predicted by the MPM was much smaller, on average, than we observed in natural populations (Figure), presumably because transplants did not grow like ``real'' plants. 
...
\begin{figure}[h!]
  \begin{center}
    \includegraphics[width=0.75\linewidth]{Figures/SR_viab}
