...
\revise{\linelabel{waic}In Appendix XX, we used WAIC-based model selection to show that vital rate models using precipitation and longitude as environmental covariates were statistically indistinguishable, which suggests that longitude is an adequate proxy for aridity.}
