...
\revise{\linelabel{waic}In Appendix XX, we used WAIC-based model selection to show that models using precipitation and longitude as environmental covariates were statistically indistinguishable, which suggests that longitude is an adequate proxy for aridity.}
...
This was motivated by the observation that most source populations came from the interior of the geographic range (MAP) and were brought to edge and beyond-edge locations that were much drier or wetter than their historical climate regime. 
