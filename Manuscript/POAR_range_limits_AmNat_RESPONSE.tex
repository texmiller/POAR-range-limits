...
\response{We experimented with adding the survey sites to the common garden map but indeed, this makes the map too busy in our opinion. Therefore, we have added a new map of survey sites to the Appendix.}
...
\response{We have softened the language here to indicate that marginal environments ``are often'' extreme relative to the range core (line \ref{ms-r1.1}).}
...
\response{Great question! Our new analyses (Appendix) suggest that environment of original did not strongly affect demography in our common gardens which leads us to conclude that sex is the stronger component of niche differentiation.}
...
\response{This is a really important observation. Based}
