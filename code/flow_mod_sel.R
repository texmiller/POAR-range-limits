...
# data
data_l <- list( n_sites  = poar.flow$site %>% n_distinct,
                n_blocks = poar.flow$block %>% n_distinct,
                site_s   = poar.flow$site,
                block_s  = poar.flow$block,
                site_block_s = data.frame( site_i  = poar.flow$site,
                                           block_i = poar.flow$block ) %>% 
                                  unique %>% .$site_i,
                male_s   = poar.flow$sex-1,
                long_s   = poar.flow$long.center,
                size_s   = poar.flow$log_size_t1,
                y_s      = poar.flow$flow_t1,
                n_s      = nrow(poar.flow) )
