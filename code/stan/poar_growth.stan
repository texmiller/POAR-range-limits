...
model {
  ...
  for (i in 1:n_g) {
  y_g[i] ~ neg_binomial_2_log(predG[i], dispG[i]);
  target += - log1m(neg_binomial_2_log_lpmf(0 | predG[i], dispG[i])); // manually zero-truncating
  }
  
  // non-ZT version would be vectorized
